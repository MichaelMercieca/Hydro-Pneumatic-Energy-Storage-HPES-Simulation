...
def calculate_initial_gas_mass_kg(
    ...
    """
    return (
        pcs_parameters.initial_absolute_pressure_pa
        * pcs_parameters.initial_gas_volume_m3
        / (
            pcs_parameters.specific_gas_constant_j_kg_k
            * pcs_parameters.initial_temperature_k
        )
    )
...
def calculate_heat_transfer_rate_w(
    ...
    """        
    return (
        pcs_parameters.heat_transfer_coefficient_w_m2_k
        * pcs_parameters.heat_transfer_area_m2
        * (
...
#       encompassing pcs_parameters and environment [PCSModel(params, env)].
def advance_pcs(
    state: HPESState,
    hydraulic_flow_rate_m3_s: float,
    gas_mass_kg: float,
    ...
    temperature_rate = calculate_temperature_rate(
        ...
        specific_heat_cv_j_kg_k=pcs_parameters.specific_heat_cv_j_kg_k
    )
    ...
    if not 0 < next_gas_volume_m3 <= pcs_parameters.total_volume_m3:
